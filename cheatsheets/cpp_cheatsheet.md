...
char s[]="hello";           // C-style String (6 elements including '\0')
...
//enum-key enum-name : enumerator-type {}; scoped enumeration
//enum enum-name : enumerator-type {};  unscoped enum
enum weekend {SAT,SUN};     // weekend is a type with values SAT and SUN
enum weekend day;           // day is a variable of type weekend
enum weekend{SAT=0,SUN=1};  // Explicit representation as int
enum {SAT,SUN} day;         // Anonymous enum
enum class Color {Red,Blue};// Color is a strict type with values Red and Blue SCOPED
...
switch (x) {                // x must be int (-> can be char)
...
[](int x) -> return_type {}; // stateless lambda expression ... use () to execute
[capture](int x) -> return_type {}; // statefull lambda expression
```
...
p-> x                       // Arrow operator (pointer_name)->(variable_name):
...
public:                     // Accessible to all
    ...
    T(): x{1} {}            // Constructor with initialization list (also x(1))
...
struct T {                  // Equivalent to: class T { public:
  ...
  virtual void g()=0;       // Must be overridden (**pure virtual**). CLass T is abstact
  virtual ~T(){}            // Virtual destructor must be specified in base T
  virtual void j() final;   // Prevents further overriding.
class U: public T {         // Derived class U inherits all members of base T
  public:                   // Override specifier for method g,
  void g(int) override; };  //   ensures overriding
class V: private T {};      // Inherited members of T become private
class V: protected T {};    // Inherited members of T become protected
class W: public T, public U {};
                            // Multiple inheritance
class X: public virtual T {};
                            // Classes derived from X have base T directly
class T final {}            // Class T can not have derived classes
...
destructors do not inherit. Private class members are inherited but not accessible.
Polymorphism in C++ means, the same entity (function or object) behaves differently in different scenarios. If the virtual funtions' type and signature do not match the base class function, the function will be overwritten and statically bound by the compiler. Works with base class pointers and references.
Abstract classes can not be instantiated directly. If a derived class does not override the pure virtual function, the derived class is also abstract.

## Templates

Include templates in .h files (do not use .cpp)

```cpp
template <class T> T f(T t);// Overload f for all types (class or typename)
...
void f(T1 a, T2 b) {
  cout << a << b;   // insertion << must be overloaded for T1 and T2
}
f<char, double>('a', 9)  // the compiler will replace with tempalte
```
...
x.use_count()               // Number of shared pointers referencing the same object
...
unique_ptr<int> q;          //  Can not be copied and assigned
z = make_unique<int>(16);   // Allocate int (16) on heap. Only one reference allowed.
q = move(z);                // Move reference (ownership) from z to q. z becomes nullptr
if (z == nullptr){
    cout << "Z null";
}
cout << *q;
...
cin.ignore(n, delim);       // Ignores n characters of until the delim
if (cin)                    // Good state (not EOF)?
                            // To read/write any type T:
istream& operator>>(istream& i, T& x) {i >> ...; x=...; return i;}
ostream& operator<<(ostream& o, const T& x) {return o << ...;}

#include <iomanip>          // necessary for manipulators, not so for methods
cout << resetioflags()      // reset io flags (basefield,showbase,showpos,...)
cout << noboolapha;         // default 0/1 boolean formattiing
cout << boolapha;           // true/false boolean formattiing
cout << showbase            // show base of digits (shownobase is default)
cout << uppercase           // print uppercase
cout << showpos             // print + sign before positive digits (noshowpos)
cout << std::dec;           // display integers as decimals
cout << std::hex;           // display integers as hexadecimal
cout << std::oct;           // display integers as octal
...
ofstream out_f {"name",  // Truncate(discard) when opening
std::ios::trunc/app/ate}   // append, seek to end stream when opening?
```
...
`.end` is the location **after** the last element!

```cpp
#include <vector>         // Include vector (std namespace)
vector<int> a(10);        // a[0]..a[9] are int (default size is 0)
vector<int> a(10, 100);   // overloaded constr a[0]..a[9] are 100 (int)
...
a.front();                // a[0];
a.begin(); // iterators - first element
a.rbegin(); // iterators -reverse
a.cbegin(); // iterators - constant
a.end(); // iterators -location after last element!
...
all_of(a.begin(), a.end(), f); // f lambda, all_of returns true if every f true
any_of(a.begin(), a.end(), f); // f lambda, all_of returns true if one f true
reverse(a.begin(), a.end()); // Reverse vector or deque
replace(a.begin(), a.end(), x, y); // Replace every x with y
accumulate(a.begin(), a.end()), 0; // sum the values, start with 0 (int) 0.0dbl
find(a.begin(), a.end(), b) // location, must have provided the operator==
count(a.begin(), a.end(), b) // count of b, must have provided the operator==
count_if(a.begin(), a.end(), f) // f lambda that returns true or false
for_each(a.begin(), a.end(), f) // applies f to each element in the iterator seq
transform(a.begin(), a.end(), begin, transf?) // begin = a.begin(), ::toupper (:: - global scope)
// f can be a functor, function pointers or lambda exp (c++11)
```
...
```
-exec ptype variable_name // See type of variable
```
